    ...
    <div className='orderList'>
      <div className='orderList-content'>
        ...
        <div className='orderList-content-middle'>
          <Notepad orderListData={checkData} type={"orderList"} />
        </div>
      ...
      <div className='orderList-footer'>
        {orderList.map((item, index) => {
          ...
          for (let i = 0; i < item.orderItem.length; i++) {
            price += item.orderItem[i].price * 1000 * item.orderItem[i].quantity;

            for (let j = 0; j < item.orderItem[i].orderItemsOptions.length; j++) {
              price += item.orderItem[i].orderItemsOptions[j].price * 1000 * item.orderItem[i].orderItemsOptions[j].quantity;
            }
          }
          return (
            <Button
              key={item.id}
              className='orderList-footer-btn'
              style={{
                marginLeft: (index === 0 || index % 6 === 0) && 0,
                marginTop: index < 6 && 0,
              }}
              onClick={() => {
                setCheckData(item);
